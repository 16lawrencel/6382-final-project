...
class CompassGaitController(VectorSystem):
    ...
    def compute_trajectory(self, state_initial, state_final, min_time, max_time):
        """
        ...
        # The following expressions were generated by compass_gait_expr.py
        ...
        def D2_L(q, qdot):
            return np.array(D2_L_raw(q[0], qdot[0], q[1], qdot[1]))
        
        def D1_Ld(k):
            return np.concatenate(D1_Ld_raw(states[k,0], states[k,1], states[k+1,0], states[k+1,1]))
        
        def D2_Ld(k):
            return np.concatenate(D2_Ld_raw(states[k,0], states[k,1], states[k+1,0], states[k+1,1]))
...
def ComputeTrajectory(state_initial, state_final, min_time, max_time):
    ...
    states, u, time_step = controller.compute_trajectory(state_initial, state_final, min_time, max_time)
    return states, u, time_step
